...
이 프로젝트는 단순 API CRUD 구현보다,  
**설치 전 유입 정보와 설치 후 앱 최초 실행 시점을 안전하게 연결하는 서버 처리 흐름 검증**에 초점을 두고 있습니다.

주요 검증 포인트는 다음과 같습니다.

- 광고 클릭 시점 식별자 발급 및 유입 상태 저장
- 암호화 토큰 기반 위변조 방지
- 최초 실행 시 access sequence 기반 요청 검증
- OS 및 app_type 기준 분기 처리
- 서버 기준 접근 이력 추적 및 운영 대응 가능성 확보

상세 검증 내용은 아래 문서를 통해 정리했습니다.
