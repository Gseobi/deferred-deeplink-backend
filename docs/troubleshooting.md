...
### Why It Matters
비모바일 접근을 정상 흐름에 포함하면
잘못된 Redirect, 검증 실패, 불필요한 운영 이슈가 발생할 수 있습니다.

### Response
본 프로젝트에서는 User-Agent 기반 OS 판별을 통해
모바일(Android / iOS) 환경만 정상 흐름으로 허용하고,
비모바일 접근은 `invalidRequest` 페이지로 분기했습니다.

### Note
User-Agent 기반 분기는 완전한 보안 수단이 아니라
정상 처리 대상 환경을 제한하는 운영 제어 장치로 보았습니다.
...
### Problem
하나의 백엔드가 여러 앱 또는 여러 광고 경로를 동시에 처리하면
앱별 스토어 URL, 앱 스킴, 랜딩 정보가 섞일 수 있습니다.

### Why It Matters
분기 기준이 불명확하면 잘못된 앱으로 연결되거나
유입 정보 관리가 복잡해질 수 있습니다.

### Response
본 프로젝트에서는 `/install/{app_type}` 구조를 사용해
앱 구분 기준을 URL 레벨에서 명확히 분리했습니다.
...
- Click ID / `crypt` / `access_seq`를 통한 상태 연결
- `app_type` 기반 다중 앱 분기
- OS 기반 접근 제어
- JPA / Querydsl 책임 분리
